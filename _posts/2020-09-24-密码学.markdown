...
加密和解密的时候都需要[密钥](agenda://note/5B06D97B-7286-4BFB-830C-9F4772B3C813)

[对称密码](agenda://note/AF96144E-AEEF-4D52-BF42-F81F60A8125C)（symmetric cryptography）：在加密和解密时使用同一种密钥的方式（别名：公共密钥密码、传统密码、私钥密码、共享密钥密码等）

[公钥密码](agenda://note/EF94C4BB-7DA2-4DBB-842A-1CE0154BBD82)（public-key cryptography）：在加密和解密的时候使用不同密钥的方式，又称非对称密码（aymmetric cryptography）

[混合密码系统](agenda://note/3CC45AFE-FEEE-471D-BAA6-A298BA668072)（hybrid cryptosystem）：将对称密码和公钥密码结合起来的密码方式


# 其他密码技术

[单向散列函数](agenda://note/C3764C11-40DB-4FB9-AC6B-0C67F5F7005C)（one-way hash function）：保证完整性（integrity）

[消息认证码](agenda://note/E4FCB003-2833-4833-8B14-CE5B3272A460)（message authentication code）：保证完整性、提供认证（authentication）

[数字签名](agenda://note/9D930DB3-1825-483B-B67C-0F6BCE6D3E69)（digital signature）：对数字签名进行验证（verify）能够确保完整性、提供认证、防止否认

[伪随机数生成器](agenda://note/7C70106F-F3A0-4A8C-A92F-83F0356C27B5)（Pseudo Random Number Generator，PRNG）：能够模拟产生随机数列的算法，承担着密钥生成的职责。如：在Web中进行SSL/TLS通信时，会用PRNG生成一个仅用于当前通信的临时密钥（会话密钥）
